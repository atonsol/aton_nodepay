...
def show_warning():
    confirm = input("THIS IS BYPASSED VERSION - BYPASSED BY ATON \nPress Enter to continue or Ctrl+C to cancel... ")
...
def load_token(token_file):
    try:
        with open(token_file, 'r') as file:
            tokenid = file.read().splitlines()
        return tokenid
    except Exception as e:
        logger.error(f"Failed to load tokenid: {e}")
        raise SystemExit("Exiting due to failure in loading tokenid")        
...
if __name__ == '__main__':
    show_warning()
    print("\nAlright, we here! Wait a momment for loading token and proxy.")
    try:
        asyncio.run(main())
